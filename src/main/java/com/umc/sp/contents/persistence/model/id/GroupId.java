...
public class GroupId implements Serializable, Comparable<GroupId> {
    ...
    @Converter
    public static class DbConverter implements AttributeConverter<GroupId, UUID> {

        @Override
        public UUID convertToDatabaseColumn(GroupId id) {
            return nonNull(id) ? id.getId() : null;
        }

        @Override
        public GroupId convertToEntityAttribute(UUID value) {
            return nonNull(value) ? new GroupId(value) : null;
        }
    }
}
